...
{
	DWORD result = XInputModuleManager::Get().XInputGetState(dwUserIndex, pState);
	...
	if (dwUserIndex != 0)
	{
		// The hack will works only for player 1
		return result;
	}

	if (pState->Gamepad.sThumbRX > 0)
	{
		// Whammy is half down, skipping the autostrum hack
		return result;
	}

	XINPUT_CAPABILITIES* pCaps = new XINPUT_CAPABILITIES;
	XInputModuleManager::Get().XInputGetCapabilities(dwUserIndex, XINPUT_FLAG_GAMEPAD, pCaps);
	
	if (pCaps->SubType == XINPUT_DEVSUBTYPE_GUITAR ||
		pCaps->SubType == XINPUT_DEVSUBTYPE_GUITAR_ALTERNATE ||
		pCaps->SubType == XINPUT_DEVSUBTYPE_GUITAR_BASS)
	{
		BOOL isGreenDown = pState->Gamepad.wButtons & XINPUT_GAMEPAD_A;
		BOOL isRedDown = pState->Gamepad.wButtons & XINPUT_GAMEPAD_B;
		BOOL isYellowDown = pState->Gamepad.wButtons & XINPUT_GAMEPAD_Y;
		BOOL isBlueDown = pState->Gamepad.wButtons & XINPUT_GAMEPAD_X;
		BOOL isOrangeDown = pState->Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER;
		BOOL isFretDown = isGreenDown || isRedDown || isYellowDown || isBlueDown || isOrangeDown;
		if (isFretDown)
		{
			// Adding strum down to the pressed buttons list
